...
public:
    vioManager()
    {
        ...
        for (int i = 1; i < (int)nImages; i++)
        {
            ...
            // [Debug] - Publishing Feature Tracking Results
            if (!prevUndistortedRGB.empty() && !currUndistortedRGB.empty())
            {
                cv::Mat vis;
                cv::hconcat(prevUndistortedRGB, currUndistortedRGB, vis);
                int offsetX = prevUndistortedRGB.cols;
                int N = std::min(prevBestPts.pts.size(), currBestPts.pts.size());
                for (int k = 0; k < N; k++)
                {
                    cv::Point2f p1 = prevBestPts.pts[k];
                    cv::Point2f p2 = currBestPts.pts[k];

                    cv::Point2f p2_shift(p2.x + offsetX, p2.y);

                    cv::circle(vis, p1, 3, cv::Scalar(0, 255, 0), -1);
                    cv::circle(vis, p2_shift, 3, cv::Scalar(0, 0, 255), -1);
                    cv::line(vis, p1, p2_shift, cv::Scalar(255, 0, 0), 1);
                }
                float scale = 0.4f;
                cv::resize(vis, vis, cv::Size(), scale, scale);

                publishDebugImage(vis);
            }
